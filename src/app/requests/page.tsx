...
if (actionType === "saved" || actionType === "accepted" || actionType === "contacted") {
...
const { error: actionError } = await supabase.from("technician_project_actions").insert({
technician_id: expertId,
project_id: requestId,
...
const { error: contactError } = await supabase.from("expert_contact_requests").insert({
...
message: "An expert has requested contact regarding your industrial support request.",
...
const urgentCount = requests.filter((request) => (request.urgency || "").toLowerCase().includes("urgent")).length;
...
<h1 className="mt-4 text-4xl font-bold text-[#111827]">Industrial Access Required</h1>
<p className="mx-auto mt-4 max-w-xl text-[#4b5563]">
To view industrial requests, you must create an account or log in first. VALCRONS protects
facilities, experts, and operational information through verified account-based access.
</p>
...
Review verified operational requests, save opportunities, accept projects, and request
facility contact through VALCRONS.
</p>
</div>
...
{loading && <p className="text-sm text-[#374151]">Loading requests...</p>}
...
<p className="mt-4 text-sm leading-7 text-[#374151]">
{request.problem_description || "No additional details provided."}
</p>
<div className="mt-6 grid gap-4 text-sm sm:grid-cols-3">
<Info
label="Company"
value={contactRequested ? request.company_name || "Not provided" : "Hidden until contact request"}
/>
<Info
label="Contact"
value={contactRequested ? request.contact_person || "Not provided" : "Protected"}
/>
<Info
label="Email"
value={contactRequested ? request.work_email || "Not provided" : "Protected"}
/>
</div>
</div>
)}
...
disabled={saved || updatingAction === `${request.id}-saved`}
...
Write a short professional message for the facility. This improves your chance of being
...
Add resume, certifications, safety training, or portfolio files. File upload storage will
be upgraded later; this version records selected file names for the facility review.
</p>
...
<p className="mt-3 text-xs text-[#6b7280]">Supported files: PDF, DOC, DOCX, JPG, PNG</p>
{attachments.length > 0 && (
<div className="mt-4 space-y-2">
{attachments.map((file) => (
<p key={file.name} className="rounded-xl bg-white px-4 py-2 text-sm font-medium text-[#111827]">
{file.name}
</p>
))}
</div>
...
disabled={!contactMessage.trim() || updatingAction === `${contactRequest.id}-contacted`}
...
>
{updatingAction === `${contactRequest.id}-contacted` ? "Sending..." : "Send Contact Request"}
</button>
</div>
</div>
</div>
...
<p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#6b7280]">{title}</p>
<p className="mt-4 text-5xl font-semibold tracking-[-0.05em] text-[#111827]">{value}</p>
<p className="mt-3 text-xs font-medium text-[#6b7280]">Expert activity metric</p>
</div>
);
}
